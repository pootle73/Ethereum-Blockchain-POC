if (Meteor.isClient) {
  ...
  Template.documentTable.helpers({
    ...
    transactions: function(){
      loggerContract.Log({}, {fromBlock:8000, toBlock:"latest"}).watch(function(a,b){
          console.log("In transactions")
        //var hash = web3.toAscii(b.args.hash)
        var doc = Document.findOne({name: b.args.logString, blockNumber: b.args.blockNumber.toString()})

        if(doc == undefined){
          Document.insert({name: b.args.logString, blockNumber: b.args.blockNumber.toString()})
        }
      })
